...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MandarinCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MandarinCoin Signed Message:\n";
...
{
    // MandarinCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
        ...
        //start with the old MandarinCoin schedule, the block rewards remain the same until we hit the hardfork
        int height1 = 200000;
        int height2 = 400000;
        int height3 = 600000;
        int height4 = 800000;
...
static const int64 nTargetTimespan = 1 * 60 * 60; // MandarinCoin: retarget every 1 hours
int64 GetTargetSpacing()
{
    return nBestHeight <= HARDFORK_HEIGHT_1 && !GetBoolArg("-testnet", false) ? 30 : 60;
}
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MandarinCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        uint64              PastBlocksMin       = 36;
        uint64              PastBlocksMax       = 1008;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MandarinCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MandarinCoin: temporarily disable v2 block lockin until we are ready for v2 transition
...
unsigned char pchMessageStart[4] = { 0xc0, 0xc0, 0xc0, 0xc0 }; // MandarinCoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * (pindexPrev->nHeight < HARDFORK_HEIGHT_1 ? 2880 : 1440) / 250)))
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MandarinCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MandarinCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MandarinCoinMiner(CWallet *pwallet)
{
    printf("MandarinCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MandarinCoin-miner");
    ...
    try { loop {
        ...
        printf("Running MandarinCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MandarinCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MandarinCoinMiner, pwallet));
}
